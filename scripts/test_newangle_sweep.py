"""Parameter sweep for new-angle red sample — windowed + pre-cached."""
...
GEOM_FILE = "samples/new-angle/sample-red.geometry.json"
SCORES_FILE = "samples/new-angle/sample-red.scores.json"
PADDING = 150
...
# Red scores → only test line-1 (red/left goal)
# Each line is an independent goal — don't combine across the field
test_configs = [(lines[0]["name"], [lines[0]])]

# Compute crop windows per config
...
# Pre-read and cache cropped frames per config
config_caches = {}
for name, line_geoms in test_configs:
    ...
    # Check if we already cached this crop
    cache_key = crop
...
BA_VALUES = [300, 500, 700, 900, 1200, 1500, 2000, 3000]
BW_VALUES = [10, 15, 20, 25, 30]
    ...
    for ba in BA_VALUES:
        for bw in BW_VALUES:
            for fr in FR_VALUES:
                ...
                # Build band mask in crop coordinates
